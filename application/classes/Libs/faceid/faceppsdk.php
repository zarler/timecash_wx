...
class Libs_faceid_faceppsdk
{
    ...
    ######################################################
    public $server          = 'http://api.faceid.com/faceid/v1';
//    public $server         = 'http://apicn.faceplusplus.com/v2';
    ...
    private function request($request_url, $request_body)
    {
        ...
        if (array_key_exists('img', $request_body)) {
            ...
            $request_body['image'] = new CURLFile($request_body['img']);
        } else {
            if (array_key_exists('image_idcard', $request_body)) {
                $request_body['image_idcard'] = '@' . $request_body['image_idcard'];
            } else {
