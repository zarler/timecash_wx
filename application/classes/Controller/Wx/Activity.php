 ...
 *
 * */
class Controller_Wx_Activity extends WxHome
{
    ...
    /************************************************************************************************************************************
     *  领券活动(TC0A-016)(button1，button2，button3 按钮)
     ************************************************************************************************************************************/
    ...
    public function action_TC0A016(){
        ...
        //浏览统计
        $ip = $this->getIp();
        if($this->_activity->get_statistics(array('ip'=>$ip,'action'=>'look','event_name'=>'TCOA_016_LOOK'),'ac_count')){
            $this->_activity->insert_statistics(array('action'=>'look','ip'=>$ip,'create_time'=>time(),'event_name'=>'TCOA_016_LOOK','reg_app'=>'wechat'),'ac_count');
        }
        $variable = array(
            "app"=>$this->getWxInfo()
        );
...
//                        $val['button'.$key] = "<a href='javascript:layerMobile.submitOk(\"您已经领取过啦，机会留给别人吧\");layerMobile.changeCssPromptMsgAtc();' class='coupon-btn'>马上使用</a>";
                        $val['button'.$key] = "<a href='/' class='coupon-btn'>马上使用</a>";
                    }
                    //0-100（100位抢完）
                    // $val['proportion'] = 100;
                }
        ...
        if(isset(Gv::$_userInfo['user_id'])&&Valid::not_empty(Gv::$_userInfo['user_id'])){
            ...
            //登录未登录
            //保存登录点
            $this->_session->sessionSet('activity','/wx/Activity/TC0A016');
            //未登录优先
            $Gol = true;
            $result['result'][0]['button1'] = '<a href="javascript:clickSubmit();layerMobile.submitUrl(\'亲，登陆后可以领券哦\',\'/Login\');layerMobile.changeCssPromptMsgAtc();" data-code="100" class="coupon-btn">点击领取</a>';
            $result['result'][1]['button2'] = '<a href="javascript:clickSubmit();layerMobile.submitUrl(\'亲，登陆后可以领券哦\',\'/Login\');layerMobile.changeCssPromptMsgAtc();" data-code="100" class="coupon-btn">点击领取</a>';
            $result['result'][2]['button3'] = '<a href="javascript:clickSubmit();layerMobile.submitUrl(\'亲，登陆后可以领券哦\',\'/Login\');layerMobile.changeCssPromptMsgAtc();" data-code="100" class="coupon-btn">点击领取</a>';
        }
        ...
        parent::$_VArray['requestUrl'] = "/wx/FunctionsAct/016GetCoupon";
        parent::$_VArray['couponList'] = $result['result'];
        parent::$_VArray['shareButton'] = 'javascript:clickSubmit();bomob_screen.showMask(true);';

        //点击统计
        parent::$_VArray['clickButtonUrl'] = '/wx/FunctionsAct/statisticsUserIdIp';
        //分享统计
        parent::$_VArray['shareUrl'] = '/wx/FunctionsAct/shareWx';
...
////        $acStatus = Tool::factory('AcTool')->TimeLimit(1510366400,1511625600,'活动未开始。','端午节活动已结束');
...
//        }
        ...
        if($this->_activity->get_statistics(array('ip'=>$this->_ip,'action'=>'look_h','event_name'=>'TCOA_017_LOOK'),'ac_count')){
            $this->_activity->insert_statistics(array('action'=>'look_h','ip'=>$this->_ip,'create_time'=>time(),'event_name'=>'TCOA_017_LOOK','reg_app'=>'wechat'),'ac_count');
        }
        ...
        if(isset($result['code']) && $result['code']==1000){
            ...
            if(isset($result['result'])&&Valid::not_empty($result['result'])){
                foreach ($result['result'] as $key =>$val){
                    ...
                    if($key>=4){
                        //显示更多
                        parent::$_VArray['listRedMore'] = "<a href='/wx/Activity/TC0A017List'>显示所有</a>";
                    }
                }
            }else{
        ...
        if(isset(Gv::$_userInfo['user_id'])&&Valid::not_empty(Gv::$_userInfo['user_id'])){
            ...
            if(isset($result['code']) && $result['code']==1000){
                parent::$_VArray['LoanButton'] = '<a href="/wx/Activity/TC0A017Redrain" class="redrain-btn">马上抢红包</a>';
            }else{
                if(isset($result['code'])){
                    $this->error($result['message']);
                    die;
            ...
            //登录未登录
            $this->_session->sessionSet('activity','/wx/Activity/TC0A017Home');
            //未登录优先
//            $Gol = true;
            parent::$_VArray['LoanButton'] = '<a href="javascript:layerMobile.submitUrl(\'亲，需要先登录后才可抓红包哦\',\'/Login\');layerMobile.changeCssPromptMsgAtc();" class="redrain-btn">马上抢红包</a>';
        }
...
//        }
        ...
        if($this->_activity->get_statistics(array('ip'=>$this->_ip,'action'=>'look_r','event_name'=>'TCOA_017_LOOK'),'ac_count')){
            $this->_activity->insert_statistics(array('action'=>'look_r','ip'=>$this->_ip,'create_time'=>time(),'event_name'=>'TCOA_017_LOOK','reg_app'=>'wechat'),'ac_count');
        }
        ...
        //删除抽奖次数
        parent::$_VArray['timeReq'] = '/wx/FunctionsAct/017DeductTimes';
        //抽奖请求
        parent::$_VArray['luckDrawReq'] = '/wx/FunctionsAct/017LuckDraw';
        parent::$_VArray['timesReqt'] = "/wx/Activity/TC0A017List";
        //邀请好友
        parent::$_VArray['invitationUrl'] = "javascript:bomob_screen.showMask(true);";
        //点击统计
        parent::$_VArray['clickButtonUrl'] = '/wx/FunctionsAct/statisticsUserIdIp';
        //分享统计
        parent::$_VArray['shareUrl'] = '/wx/FunctionsAct/shareWx';
    ...
    public function action_TC0A017List(){
        ...
        if($this->_activity->get_statistics(array('ip'=>$this->_ip,'action'=>'look_l','event_name'=>'TCOA_017_LOOK'),'ac_count')){
            $this->_activity->insert_statistics(array('action'=>'look_l','ip'=>$this->_ip,'create_time'=>time(),'event_name'=>'TCOA_017_LOOK','reg_app'=>'wechat'),'ac_count');
        }
    ...
    //点赞
    public function action_TC0A017Zan(){


        if($this->_activity->get_statistics(array('ip'=>$this->_ip,'action'=>'look_z','event_name'=>'TCOA_017_LOOK'),'ac_count')){
            $this->_activity->insert_statistics(array('action'=>'look_z','ip'=>$this->_ip,'create_time'=>time(),'event_name'=>'TCOA_017_LOOK','reg_app'=>'wechat'),'ac_count');
        }
        ...
        $json_info = json_encode($variable);
        $result = $this->_api->getApiArrays('AC_TCOA017','HaveFllow','',array('json'=>$json_info));
//        Tool::factory('Debug')->D(array($json_info,$variable,$result));

        if(isset($result['code']) && $result['code']==1000){
            if(isset($result['result']['have'])&&$result['result']['have']==0){
                //没有赞过
                parent::$_VArray['have'] = '<div class="pop-wrap js-mask" style="display: block;"><div class="redrain-pop-mask"></div><div class="redrain-pop-zan"><div class="redrain-pop-up"></div><a href="javascript:Zan();"></div></div>';
                //请求点赞地址
                parent::$_VArray['zanReq'] = '/wx/FunctionsAct/017LuckZan';
                parent::$_VArray['userId'] = $_GET['userId'];
            }else{
                //赞过
                parent::$_VArray['have'] = '<div class="pop-wrap js-mask" style="display: block;"><div class="redrain-pop-mask"></div><div class="redrain-pop"><div class="redrain-pop-done"></div></div></div>';
            }
        }else{
            if(isset($result['code'])){
                $this->error($result['message']);
        ...
        $view = View::factory($this->_vv.'Activity/TC0A_017_Zan');
        $view->_VArray = parent::$_VArray;
        $this->response->body($view);
    }
...
//                    parent::$_VArray['buttonRepayStr'] = '<a href="/Account/BorrowingRecords#jump=BannerLoanRecord" class="lottery-pop-share-btn">马上还款</a>';
                ...
                //测试
//                parent::$_VArray['buttonStr'] = '<button onclick="submit();"  id="lottery-btn">马上抽奖</button>';
            }else{
                if(isset($result['code'])){
                    $this->error($result['message']);
                    die;
            ...
            //登录未登录
            //保存登录点
            $this->_session->sessionSet('activity','/wx/Activity/TC0A019');
            parent::$_VArray['buttonStr'] = '<button onclick="javascript:layerMobile.submitUrl(\'亲，需要先登录后才可抓红包哦\',\'/Login\');layerMobile.changeCssPromptMsgAtc();" id="lottery-btn">马上抽奖</a>';
            //未登录优先
        }
        //点击统计
        parent::$_VArray['019LuckDraw'] = '/wx/FunctionsAct/019LuckDraw';
        //邀请好友
        parent::$_VArray['invitationUrl'] = "javascript:clickSubmit('click_i');bomob_screen.showMask(true);";
    ...
    public function action_TC0A019Share(){
        ...
        //滚动条
        $arr=array('133****2206','134****3254','180****2788','150****3589','138****0710','187****7699','159****2611','182****8007','180****0163','185****3530','158****9271','183****8166','188****6490','182****1599','136****8685','158****3779','183****0176','186****6671','136****2001','159****0961');
        shuffle($arr);
        $arr = array_slice($arr,0,5);
//        $arr = null;
        if(Valid::not_empty($arr)){
            parent::$_VArray['listStr'] = null;
            foreach ($arr as $key =>$val){
                parent::$_VArray['listStr'].= "<li>恭喜{$val}  获得提额</li>";
            }
        }else{
            parent::$_VArray['listStr'] = '<p style="text-align: center">暂无获奖信息</p>';
        }

        parent::$_VArray['buttonStr'] = '';

        //显示弹框类型
        parent::$_VArray['tipShare'] = false;
...
//        $time = date('Y-m-d',$strr);
        ...
        parent::$_VArray['reqUrl'] = '/wx/Activity/TC0A020List';
...
//        $time = date('Y-m-d',$strr);
        ...
        parent::$_VArray['wx'] = true;
        parent::$_VArray['signPackage'] = $this->signPackage();
        parent::$_VArray['sharetitle'] = "快金元旦有豪礼，快来领500元现金!";
        parent::$_VArray['text'] = "元旦乐翻天，快金500元现金豪礼等你领！";
        parent::$_VArray['img_url'] = Kohana::$config->load('url.communic_url.timecash_m')."static/images/promotion/icon_logo.png";
        parent::$_VArray['url'] = Kohana::$config->load('url.communic_url.timecash_m')."wx/Activity/TC0A021";


        parent::$_VArray['resultAtt'] = $resultAtt;
        parent::$_VArray['reqUrl'] = '/wx/Activity/TC0A021List';
        //分享统计
        parent::$_VArray['shareUrl'] = '/wx/FunctionsAct/shareWx';
...
//        $time = date('Y-m-d',$strr);
    ...
    public function action_StagingExtension(){
        ...
        if(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone')||strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')){
            ...
            parent::$_VArray['client'] = "/Promotion/InstandroidUpload";
...
//        parent::$_VArray['resultAtt'] = $resultAtt;
     ...
     * 耀贷注册
     ************************************************************************************************************************************/
    public function action_TCOA022(){
//        Tool::factory('Debug')->D(parent::$_VArray);
//        parent::$_VArray['resultAtt'] = $resultAtt;
        $view = View::factory($this->_vv.'Activity/TC0A_022');
        $view->_VArray = parent::$_VArray;
        $this->response->body($view);
    }
    ...
    public function action_TCOA025(){
        ...
        if(isset(Gv::$_userInfo['user_id'])&&Valid::not_empty(Gv::$_userInfo['user_id'])){
            $this->_RecordVisit = array('action'=>'look', 'event_name'=>'TCOA_025_LOOK', 'reg_app'=>'wechat', 'table'=>'ac_count');
            //流量统计(按照用户id统计)
            if($this->_activity->get_statistics(array('user_id'=>Gv::$_userInfo['user_id'],'action'=>'look','event_name'=>'TCOA_025_LOOK','reg_app'=>'wechat'),'ac_userid_count')){
                $this->_activity->insert_statistics(array('user_id'=>isset(Gv::$_userInfo['user_id'])?Gv::$_userInfo['user_id']:null,'action'=>'look','ip'=>$this->_ip,'create_time'=>time(),'event_name'=>'TCOA_025_LOOK','reg_app'=>'wechat'),'ac_userid_count');
            }
            parent::$_VArray['buttonStr'] = "javascript:clickSubmit();location.href='/wx/QuickReceipts/Index';";
        }else{
            //未登录
            $this->_session->sessionSet('activity','/wx/Activity/TCOA025');
            parent::$_VArray['buttonStr'] = 'javascript:clickSubmit();layerMobile.submitUrl(\'亲，需要您先登录哦！\',\'/Login\');layerMobile.changeCssPromptMsgAtc();';
        }
        //点击统计
        parent::$_VArray['clickButtonUrl'] = '/wx/FunctionsAct/statisticsUserIdIp';
    ...
    public function action_TCOA026(){
        ...
        //流量统计(按照ip统计)
        $this->_RecordVisit = array('action'=>'look_h', 'event_name'=>'TCOA_026', 'reg_app'=>'wechat', 'table'=>'ac_count');
        $this->_activity->insert_statistics(array('user_id'=>isset(Gv::$_userInfo['user_id'])?Gv::$_userInfo['user_id']:null,'action'=>$this->_RecordVisit['action'],'ip'=>$this->_ip,'create_time'=>time(),'event_name'=>$this->_RecordVisit['event_name'],'reg_app'=>$this->_RecordVisit['reg_app']),$this->_RecordVisit['table']);

//      Tool::factory('Debug')->D(Gv::$_userInfo);
        parent::$_VArray['invitedList'] = null;
        //我的奖品
        parent::$_VArray['UrlMyPrize'] = '/wx/Activity/TCOA026MyPrize';
        if(isset(Gv::$_userInfo['user_id'])&&Valid::not_empty(Gv::$_userInfo['user_id'])){
            //登录
            parent::$_VArray['Login'] = true;
            //马上邀请好友
            parent::$_VArray['UrlInvitation'] = 'javascript:clickSubmit();bomob_screen.showMask(true);';
            //邀请详情
            parent::$_VArray['Urldetails'] = '/wx/Activity/TCOA026Record#';
            //领取请求地址
            parent::$_VArray['requestUrl'] = '/wx/FunctionsAct/026MyPrice';
            //提现(在活动期间不能体现)
            ...
            //点击统计
            parent::$_VArray['clickButtonUrl'] = '/wx/FunctionsAct/statisticsUserIdIpAll';
            //分享统计
            parent::$_VArray['shareUrl'] = '/wx/FunctionsAct/shareWxAll';

            //分享链接
//            parent::$_VArray['UrlShare'] = '/h5/Activity/TCOA026Raiders?userId='.Gv::$_userInfo['user_id'];
            parent::$_VArray['UrlShare'] = Kohana::$config->load('url.communic_url.timecash_m').'h5/Activity/TCOA026Raiders?userId='.Gv::$_userInfo['user_id'];
            //主页信息
            $variable = array(
                "app"=>$this->getWxInfo()
            );
...
//                                parent::$_VArray['prizeList'] .= '<div><span class="span-o">'.$val['prize'].'</span><span class="span-s">借款人数满'.$val['num'].'人</span><button data-code="'.$val['id'].'" onclick="javascript:getPrice(this)">领取奖品</button></div>';
            ...
            parent::$_VArray['wx'] = true;
            parent::$_VArray['signPackage'] = $this->signPackage();
            parent::$_VArray['sharetitle'] = "好友送你88元现金，iPhone X免费拿!";
            parent::$_VArray['text'] = "急用钱，找快金！送你一张7天免息券！免息用7天，当天可放款！更有iPhoneX免费拿！速戳>！";
            parent::$_VArray['img_url'] = Kohana::$config->load('url.communic_url.timecash_m')."static/Activity/TC0A-026/images/ix.png";
            parent::$_VArray['url'] = Kohana::$config->load('url.communic_url.timecash_m')."h5/Activity/TCOA026BeInvited?userId=".Gv::$_userInfo['user_id'];
         }else{
            $variable = array(
                "app"=>$this->getWxInfo()
            );
    ...
    public function action_TCOA026Record(){
        ...
        parent::$_VArray['urlWithdrawals'] = '/wx/Activity/TCOA026';
        //主页信息
        $variable = array(
            "app"=>$this->getWxInfo()
        );
