 ...
 *
 * */
class Controller_App_CreditFlow extends AppHome
//class Controller_App_CreditFlow extends Common
{
     ...
     *注册紧急联系人(3.2.0)
     *******************************************************/
    public function action_EmergencyContactsNew(){
        //通过下面几个字段来判断是否完成基本授信(跳出)
//        if(!Valid::not_empty(self::$arr_step)){
//            //异常错误
//            $this->error(Kohana::message('wx','abnormal_error'));
...
//        $view = View::factory($this->_vv.'Register/contacts');
        $view = View::factory($this->_vv.'Register/contactsnew');
        //请求地址
...
//            $this->response->body($view);
    ...
    public function action_MNO_TCredit_Return(){
        ...
        $this->redirect('/app/CreditFlow/CreditQuery_CountDown_Last');
        die;
...
//            $this->response->body($view);
    ...
    public function action_CreditQuery_CountDown_Last(){
        ...
        if(isset($_GET['tc_no'])&&Valid::not_empty($_GET['tc_no'])){
            //天创
            parent::$_VArray['tc_no'] =  $_GET['tc_no'];
        }else if(isset($_GET['outUniqueId'])&&Valid::not_empty($_GET['outUniqueId'])){
            //天机
            parent::$_VArray['outUniqueId'] =  $_GET['outUniqueId'];
        }else{
            $this->error('非法请求！');
            die;
        }
        parent::$_VArray['requestUrl'] = "/app/Functions/CreditQuery_TakeTurns";
        parent::$_VArray['requestjump'] = "/app/CreditFlow/CreditQuery_CountDown_Result";
        ...
        if(isset($_GET['jump'])){
            switch ($_GET['jump']){
                case 'overtime_3':
                    ...
                    $view->submit = '点击重新获取';
                    if(isset($_GET['tc_no'])){
                        $view->url = '/app/CreditFlow/CreditQuery_CountDown_Last?jump=overtime_2&&tc_no='.$_GET['tc_no'];
                    }elseif(isset($_GET['outUniqueId'])){
                        $view->url = '/app/CreditFlow/CreditQuery_CountDown_Last?jump=overtime_2&&outUniqueId='.$_GET['outUniqueId'];
                    }
                    break;
                case 'overtime_2':
                    //超时
                    ...
                    $view->submit = '重试';
                    $view->url = '/app/CreditFlow/MNO_Rong360_Return?jump=DynamicCode';
                    break;
                case 'pass':
                    $view->title = '审核通过';
