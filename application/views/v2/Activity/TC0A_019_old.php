...
<article>
    ...
    <div id="lottery">
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td class="lottery-unit lottery-unit-3"><img src="/static/Activity/TC0A-019/images/lottery-4.png"></td>

                <td class="lottery-unit lottery-unit-2"><img src="/static/Activity/TC0A-019/images/lottery-3.png"></td>
            </tr>
        </table>
    </div>
    ...
    <div class="lottery-info">
        ...
        <div class="lottery-use-rule">
            <p>1.活动时间：2017.12.11-2017.12.28</p>
            <p>2.活动期间，所有在快金完成极速贷还款的用户都可进行抽奖，获得不同额度的提额，100%中奖 （讨论逾期用户是否和抽奖）</p>
            <p>3.获奖后在快金极速贷首页查看获得提额。下笔借款可直接使用</p>
            <p>4.提额有效期是到账后的14天，活动期间多次抽奖获得提额可累加</p>
            <p>5.提额只在快金借款时使用，不能提现及作为其他</p>
...
<?php if(isset($_VArray['share'])&&Valid::not_empty($_VArray['share'])){ ?>
    ...
    <div class="pop-wrap js-mask1" style="display: block;">
        ...
        <div class="lottery-pop-content recevie">
            ...
            <a href="/#jump=BannerLoanRecord" class="lottery-pop-share-btn">马上还款</a>
        </div>
    </div>
...
<?php if(isset($_VArray['share'])&&Valid::not_empty($_VArray['share'])){ ?>
    ...
    <script type="text/javascript">
        ...
        window.onload=function(){
            ...
            $("#lottery-btn").click(function(){
                $('#lottery-btn').attr('id','lottery-btn-grep');
                cansubmit = false;
                $.ajax({
                    ...
                    success: function (result) {
                        if(result.status){
                            index = result.code;
                            $('#winning .lottery-pop-txt h3').html('恭喜!获得'+result.prize+'元提额');
                            console.log(result);
                            if (click) {
                                return false;
                            }else{
    ...
    <?php if(isset($_VArray['app'])&&$_VArray['app']){ ?>
        <script>
            ...
            function shareWx(){
                if(isAndroid){
                    window.android.sharingFriends('<?php echo $_VArray['sharetitle']; ?>', '<?php echo $_VArray['text']; ?>', '<?php echo $_VArray['img_url']; ?>', '<?php echo $_VArray['url']; ?>','18');
                }
                if(isiOS){
                    sharingFriends('<?php echo $_VArray['sharetitle']; ?>', '<?php echo $_VArray['text']; ?>', '<?php echo $_VArray['img_url']; ?>', '<?php echo $_VArray['url']; ?>','18');
                }
            }
        </script>
        ...
        <script>
            ...
            function shareAjax() {
                $.ajax({
                    ...
                    type:'POST',
                    data:{action:'show',event_name:'TCOA_017',time:0},
                    dataType:'json',
                    async: true,
                    beforeSend:function(){
                    },
