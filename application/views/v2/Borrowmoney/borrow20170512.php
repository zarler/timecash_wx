	...
	<div class="t-withdraw21">
		<div class="t-withdraw2">
			<div id="js-example-change-value" class="js-change-value">
				<input name="money" type="range" min="<?php echo ceil($rule['min_amount']);?>" step="<?php echo ceil($rule['amount_step_line']);?>" max=<?php echo ceil($rule['max_amount']);?> value="<?php echo ceil($rule['min_amount']);?>" data-rangeslider="" style="position: absolute; width: 1px; height: 1px; overflow: hidden; opacity: 0;">
		...
		<p class="t-withdraw1 t-bbn borrowmoney t-withdraw6"><span style="float: left"><em><?php echo ceil($rule['min_amount']);?></em>元</span><span><em><?php echo ceil($rule['max_amount']);?></em>元</span></p>
		<div class="clear"></div>
	</div>
	<div class="t-withdraw21 t-withdraw1-1" style="border-bottom:none !important;">
		<div class="t-withdraw2">
			<div id="js-example-change-value-2" class="js-change-value">
				<input type="range" name="day" step="<?php echo ceil($rule['day_step_line']);?>" min="<?php echo empty($rule['min_day'])?7:$rule['min_day'];?>" max="<?php echo empty($rule['max_day'])?7:$rule['max_day'];?>" value="<?php echo empty($rule['min_day'])?7:$rule['min_day'];?>" data-rangeslider-2="" style="position: absolute; width: 1px; height: 1px; overflow: hidden; opacity: 0;">
				<div class="t-jinbi" style="display: none">
		...
		<p class="t-withdraw1 t-bbn borrowday t-withdraw6"><span style="float: left"><em><?php echo empty($rule['min_day'])?7:$rule['min_day'];?></em>天</span><span><em><?php echo empty($rule['max_day'])?7:$rule['max_day'];?></em>天</span></p>
		<div class="clear"></div>
	</div>
</section>
<section class="t-login-center hidden t-withdraw1">
...
<section class="t-login-footer">
	<p class="t-error"></p>
	<input type="submit" class="BR_ATF_ACTION t-orange-btn position_bottom t-orange-btn-but" value="确定"><br><br>
</section>
