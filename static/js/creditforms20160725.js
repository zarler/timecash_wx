     ...
     $(function(){
		...
		$('.t-pwd-code').click(function(){
         ...
         $(".t-red-company").click(function(){
             ...
             var comaddress_province2 = $("#province2").val();
             var comaddress_city2 = $("#city2").val();
             var comaddress_district2 = $("#district2").val();
             var address = comaddress_province2+','+comaddress_city2+','+comaddress_district2;
             var detailaddress=$("input[name='detailaddress']").val();
             var numbertell=$("input[name='numbertell']").val()?$("input[name='numbertell']").val():null;
             ...
             //    return false;
             //}
             //if(commonUtil.authcode(authcode,2)!=true){
             //    commonUtil.unlock();
             //    return false;
             //}
             if(commonUtil.isnull(address,'家庭所在地不能为空')!=true) {
                 commonUtil.unlock();
                 return false;
             }
             if(commonUtil.details_two(detailaddress,'family')!=true) {
                 commonUtil.unlock();
                 return false;
             }

             if(numbertell!=null){
                 if(commonUtil.tell(numbertell)!=true) {
                     commonUtil.unlock();
                     return false;
                 }
             }
         ...
         $('.t-button').click(function(){
                 ...
                 if(type=='sj'){
                     ...
                     if(commonUtil.passphone(passphone,'密码必须是6-8位整数')!=true) {
                         commonUtil.unlock();
                         return false;
                     }
                     if(identity == 'yd'){
                         var comauthcode=$("input[name='comauthcode']").val();
                         if(commonUtil.authcode(comauthcode,3)!=true) {
                             commonUtil.unlock();
                             return false;
                         }
                     }else{
                         //动态验证码
                         var dynamiccode=$("input[name='dynamiccode']").val();
                         if(commonUtil.isnull(dynamiccode,'')==true){
                 ...
                 if(identity == 'yd'){
                     ...
                     post_data="&phone="+phone+"&passphone="+passphone+"&comauthcode="+comauthcode;
                 }else{
                     url = creditzhurl;
                     if(type=='sj'){
         ...
         $('.t-pwd-code1').click(function(){
             ...
             if(sendtime == 0){
                 ...
                 url = dynamicSendurl;
                 post_data="&mobile="+phone+"&service_password="+passphone;
             }else{
                 ...
                 success:function(result){
                     if(result.status==true){
                         ...
                         timer = setInterval(function() {
                             comtimes--;
                             if(comtimes > 0) {
                                 $('.t-pwd-code1').text(comtimes +'秒后重发');
                                 $('.t-pwd-code').attr('disabled','disabled');
                             } else {
                                 comtimes = 60;
